...
case 10
if dateadd("h",24,shijian)>now() then
formatdate=datediff("h",shijian,now())&"小时前"
else
formatdate=datediff("d",shijian,now())&"天前"
end if
end select
end function
	...
	if catetype=3 then '外部链接
		...
		if isStatic="0" then
			select case modeltype
				case 0  '文字信息模型
					if catetype=1 or catetype=0 then					
						companyURL="news.asp?id="&cateid					
					else 'catetype=0 or 2 单页或栏目首页
						companyURL="company.asp?id="&cateid
					end if			
				case 1 '
					companyURL="products.asp?id="&cateid	
				case 2 '
					companyURL="Business.asp?id="&cateid	
				case 3
					companyURL="contact.asp?id="&cateid				
				case 4
					companyURL="lawyers.asp?id="&cateid	
				case 5
					companyURL="bbs.asp?id="&cateid	
				case 6
					companyURL="job.asp?id="&cateid	
				case else
					companyURL="company.asp?id="&cateid
			end select
		else '静态			
