...
%>
       <li class="newslist"><div class="con"><span class="time"><%=formatdate(NewsListRs("posttime"),2)%></span><a href="<%=infoURL(NewsListRs("staticUrl"),NewsListRs("dynamicUrl"),supportHtml)%>">·<%=leftstr(NewsListRs("title"),70)%></a>
       </div>
       </li>
       
<%
		
		if TrNum mod 5 = 0 then
		response.write "<li class=""split""></li>"
		end if
...
%>
       <li class="newslist"><div class="con"><span class="time"><%=formatdate(NewsListRs("posttime"),2)%></span><a href="bbsshow.asp?id=<%=NewsListRs("id")%>">·<%=leftstr(NewsListRs("title"),70)%></a>
       </div>
       </li>
...
%>
       <div class="grid_4">
            <div class="gallery-post">
                <div class="post-img">
                  <a href="readnews.asp?id=<%=NewsListRs("id")%>" class="gall_item"><span class="img-span"><i class="fa fa-arrows-alt"></i></span>
                    <img src="<%=NewsListRs("defaultpicurl")%>" alt="post image">
                  </a>
                </div>
              <div class="post-info border-box">
                <a href="readnews.asp?id=<%=NewsListRs("id")%>" class="simple-link"><%=NewsListRs("title")%></a>
                <p><%=NewsListRs("content_zy")%></p>
              </div>
            </div>
          </div>
...
Function ShowProduct()
	...
	cateid = strToNum(Request("id"))	
	Keyword = ChkFormStr(Request("so"))
	msql="Select * from product where isdel=0 and passed=True "
	If cateid<>0 Then
		'msql=msql&" and cateid="&cateid&""
		msql=msql&" and cateid in ("&sonid&")"
	End If
	...
	If 	NewsListRs.Eof Then
		Response.Write "<br />&nbsp;&nbsp;&nbsp;没有找到信息!<br /><br /><br /><br /><br />&nbsp;&nbsp;&nbsp;"
	Else
			Response.Write ("<div class=""pm3-container ""><ul>")&vbcrlf
		For TrNum=1 To mypage.pagesize
			If NewsListRs.Eof Then Exit For
...
%>
<li>
 	<div class="pic"><a href="productShow.asp?id=<%=NewsListRs("id")%>"><img src="<%=NewsListRs("defaultpicurl")%>" alt="" /></a></div>
 	<div class="tit"><a href="productShow.asp?id=<%=NewsListRs("id")%>"><%=NewsListRs("title")%></a><%=NewsListRs("productNo")%></div>
 	<div class="price">￥ <%=NewsListRs("price")%></div>
 </li>
...
<%
		...
		Set NewsListRs = Nothing
		If TrNum >=1 Then
			Response.Write(""&mypage.showpage()&"</div>")
		End If
	End If
