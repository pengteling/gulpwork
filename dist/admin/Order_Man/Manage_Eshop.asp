...
if id > 0 then
   conn.execute("update OrderList set isdel=true where id=" & id)
end if

flag="尚未处理"
set rs=server.createobject("adodb.recordset")
sqltext="select * from OrderList where isdel=false  order by addtime desc"

if keywords<>"" then
strFileName=strFileName&"&keywords="&server.URLEncode(keywords)
sqltext="select * from OrderList where isdel=false and (ordernum like '%"&keywords&"%' or productname like '%"&keywords&"%' or username like '%"&keywords&"%' or mobile  like '%"&keywords&"%' or tel like '%"&keywords&"%' or list_tr  like '%"&keywords&"%') order by addtime desc"
end if
